...
function getFormContext(el) {
  ...
  const formKey = formEl && formEl !== document.body ? hashCode(getDOMPath(formEl)) : "";
...
function resolveKey(el, binding) {
  ...
  // 1. 显式传入
  if (typeof binding.value === "string" && binding.value) return route + "|" + binding.value;
      ...
      switch (e.key) {
        case "ArrowDown":
          ...
          dropdown.selectedIndex =
            dropdown.selectedIndex < len - 1 ? dropdown.selectedIndex + 1 : 0;
          break;
        case "ArrowUp":
          e.preventDefault();
          dropdown.selectedIndex =
            dropdown.selectedIndex > 0 ? dropdown.selectedIndex - 1 : len - 1;
          break;
        case "Escape":
          e.preventDefault();
          closeDropdown();
